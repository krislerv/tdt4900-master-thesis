...
class IIRNNDataHandler:
    ...
    def get_next_batch(self, dataset, dataset_session_lengths, timestamp_set, timestamp_bucket_ids_set):
        ...
        for user in user_list:
            ...
            """
            oldest_index = session_index - self.MAX_SESSION_REPRESENTATIONS
            padding = 0

            if oldest_index < 0:
                padding = -1 * oldest_index
                oldest_index = 0

            for i in range(oldest_index, session_index):
            """
        ...
        try:
            ...
            for session_id in range(len(last_sessions_for_user)):
                for a in range(len(intra_attn_weights[user_index][session_id])):
                    file.write(str(intra_attn_weights[user_index][session_id][a].data[0]) + ",")
                file.write("\n")
                for event_id in range(len(last_sessions_for_user[session_id])):
                    file.write(str(last_sessions_for_user[session_id][event_id][1]) + ",")
                file.write("\n\n")
